       ...
       <div className="p-6">
            <Section title="Global Settings">
                <div className="flex flex-col space-y-2">
                    ...
                    <button onClick={onDeleteBubble} disabled={bubbleCount <= 1} className="w-full flex items-center justify-center space-x-2 px-4 py-2 bg-red-600 hover:bg-red-500 rounded-md transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed" title={bubbleCount <= 1 ? "Cannot delete the last bubble" : "Delete the active bubble"}>
                ...
                <div className="flex flex-col space-y-2 pt-4 border-t border-gray-700 mt-4">
                    <label htmlFor="image-upload" className={`w-full cursor-pointer flex items-center justify-center space-x-2 px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-md transition-colors ${hasVideo ? 'opacity-50 cursor-not-allowed' : ''}`} title={hasVideo ? "Clear video background to upload an image" : "Upload background image"}>
                        <UploadIcon className="w-5 h-5" />
                        <span>Image</span>
                    </label>
                    <input id="image-upload" type="file" accept="image/*" onChange={onImageUpload} className="hidden" disabled={hasVideo}/>

                    <label htmlFor="video-upload" className={`w-full cursor-pointer flex items-center justify-center space-x-2 px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-md transition-colors ${hasImage ? 'opacity-50 cursor-not-allowed' : ''}`} title={hasImage ? "Clear image background to upload a video" : "Upload background video"}>
