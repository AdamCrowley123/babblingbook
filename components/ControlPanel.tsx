...
const RichTextEditor: React.FC<{ value: string; onChange: (value: string) => void }> = ({ value, onChange }) => {
    ...
    return (
        <div className="relative">
            <div className="flex items-center flex-wrap gap-2 mb-2 p-1 bg-stone-800 rounded-md">
                <button onClick={() => execCommand('bold')} className="px-3 py-1 font-bold rounded-md hover:bg-stone-700 transition-colors">B</button>
                <button onClick={() => execCommand('italic')} className="px-3 py-1 italic rounded-md hover:bg-stone-700 transition-colors">I</button>
                <input
                    type="color"
                ...
                />
                 <select
                    ...
                    className="px-2 py-1 bg-stone-700 border border-stone-600 rounded-md hover:bg-stone-600 transition-colors"
                    defaultValue=""
            ...
            <div
                ...
                className="w-full mt-1 p-2 bg-stone-700 border border-stone-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition overflow-y-auto"
...
const ColorInput: React.FC<{ label: string; value: string; onChange: (value: string) => void;}> = ({label, value, onChange}) => (
     <div>
        ...
        <input id={label} type="color" value={value} onChange={(e) => onChange(e.target.value)} className="w-full mt-1 p-1 h-10 bg-stone-700 border border-stone-600 rounded-md cursor-pointer"/>
    </div>
);
...
const ControlPanel: React.FC<ControlPanelProps> = ({ bubbleProps, onUpdate, nextTailId, availableFonts, allBubbles, onAddConnectedBubble }) => {
  ...
  return (
    <div className="bg-stone-900 h-full flex flex-col">
       <div className="flex-grow flex flex-col overflow-hidden">
            <div className="flex-shrink-0 border-b-2 border-indigo-500">
                <nav className="flex space-x-2 px-6">
                    <button onClick={() => setActiveTab('text')} className={`px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'text' ? 'text-white border-b-2 border-white' : 'text-stone-400 hover:text-white'}`}>TEXT</button>
                    <button onClick={() => setActiveTab('bubble')} className={`px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'bubble' ? 'text-white border-b-2 border-white' : 'text-stone-400 hover:text-white'}`}>BUBBLE</button>
                </nav>
            </div>
            ...
            <div className="flex-grow p-6 overflow-y-auto">
                {activeTab === 'text' && (
                    <div id="text-panel">
                           ...
                           <RichTextEditor key={bubbleProps.id} value={bubbleProps.text} onChange={(newText) => onUpdate({ text: newText })}/>
                            ...
                            <ColorInput label="Default Text Color" value={bubbleProps.textColor} onChange={(v) => onUpdate({ textColor: v })} />
                             <div>
                                ...
                                <div className="flex mt-1 space-x-1 bg-stone-700 p-1 rounded-md">
                                    {TEXT_ALIGN_OPTIONS.map(({ value, label }) => (
                                    <button key={value} onClick={() => onUpdate({ textAlign: value })} className={`w-full py-1 rounded transition-colors text-sm ${ bubbleProps.textAlign === value ? 'bg-indigo-600 text-white' : 'bg-stone-700 hover:bg-stone-600' }`}>
                                        {label}
                                    </button>
                                    ))}
                                </div>
                            ...
                            <Slider label="Letter Spacing" value={bubbleProps.letterSpacing} min={-10} max={50} onChange={(v) => onUpdate({ letterSpacing: v })} unit="px" />
                        </Section>
                        ...
                        <Section title="Onomatopoeia Effects">
                            <div className="space-y-3 p-3 bg-stone-800 rounded-lg">
                                <Slider
                                    label="Char. Zoom Random"
                                    value={bubbleProps.charZoomRandomness}
                                    min={0} max={1} step={0.05}
                                    onChange={(v) => onUpdate({ charZoomRandomness: v })}
                                    unit="x"
                                />
                                <Slider
                                    label="Char. Y-Offset Random"
                                    value={bubbleProps.charYRandomness}
                                    min={0} max={50}
                                    onChange={(v) => onUpdate({ charYRandomness: v })}
                                    unit="px"
                                />
                                <Slider
                                    label="Char. Rotation Random"
                                    value={bubbleProps.charRotationRandomness}
                                    min={0} max={90}
                                    onChange={(v) => onUpdate({ charRotationRandomness: v })}
                                    unit="°"
                                />
                                <button
                                    onClick={() => onUpdate({ charZoomRandomness: 0, charYRandomness: 0, charRotationRandomness: 0 })}
                                ...
                                <p className="text-xs text-stone-400 pt-2 border-t border-stone-700">Adds random variation to each character. Great for sound effects!</p>
                            </div>
                        </Section>
                    ...
                    <div id="bubble-panel">
                             ...
                             {bubbleProps.shape === ShapeType.FREEHAND && (
                                <div className="space-y-3 p-3 bg-stone-800 rounded-lg">
                                    ...
                                    <button
                                        ...
                                        className="w-full flex items-center justify-center space-x-2 px-4 py-2 bg-stone-600 hover:bg-stone-500 rounded-md transition-colors text-sm"
                                    >
                                        <ResetIcon className="w-4 h-4" />
                        ...
                        <Section title="Tail & Transform">
                             <div>
                                ...
                                <input id="rotation" type="range" min="-180" max="180" value={bubbleProps.rotation} onChange={(e) => onUpdate({ rotation: parseInt(e.target.value) })} className="w-full mt-1 h-2 bg-stone-700 rounded-lg appearance-none cursor-pointer" />
                                <button onClick={() => onUpdate({ rotation: 0 })} className="p-1.5 hover:bg-stone-700 rounded-md" title="Reset Rotation">
                                    <ResetIcon className="w-4 h-4" />
                                </button>
                                </div>
                            </div>
                            ...
                            {bubbleProps.tailVisible && (
                                <div className="space-y-4 pt-3 border-t border-stone-700">
                                    {bubbleProps.tails.map((tail, index) => (
                                    <div key={tail.id} className="p-3 bg-stone-800 rounded-lg space-y-3">
                                        <div className="flex justify-between items-center">
                                            <h4 className="font-semibold text-stone-300">Tail {index + 1}</h4>
                                            <button
                                        ...
                                        <div>
                                            ...
                                            <select id={`tail-type-${tail.id}`} value={tail.type} onChange={e => handleUpdateTail(tail.id, { type: e.target.value as TailType })} className="w-full mt-1 p-2 bg-stone-700 border border-stone-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
