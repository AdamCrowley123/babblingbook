...
const RichTextEditor: React.FC<{ value: string; onChange: (value: string) => void }> = ({ value, onChange }) => {
    ...
    return (
        <div className="relative">
            <div className="flex items-center flex-wrap gap-2 mb-2 p-1 bg-slate-800 rounded-md">
                <button onClick={() => execCommand('bold')} className="px-3 py-1 font-bold rounded-md hover:bg-slate-700 transition-colors">B</button>
                <button onClick={() => execCommand('italic')} className="px-3 py-1 italic rounded-md hover:bg-slate-700 transition-colors">I</button>
                <input
                    type="color"
                ...
                />
                 <select
                    ...
                    className="px-2 py-1 bg-slate-700 border border-slate-600 rounded-md hover:bg-slate-600 transition-colors"
                    defaultValue=""
            ...
            <div
                ...
                className="w-full mt-1 p-2 bg-slate-700 border border-slate-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition overflow-y-auto"
...
const ColorInput: React.FC<{ label: string; value: string; onChange: (value: string) => void;}> = ({label, value, onChange}) => (
     <div>
        ...
        <input id={label} type="color" value={value} onChange={(e) => onChange(e.target.value)} className="w-full mt-1 p-1 h-10 bg-slate-700 border border-slate-600 rounded-md cursor-pointer"/>
    </div>
);
...
const Slider: React.FC<{ label: string; value: number; min: number; max: number; step?: number; onChange: (value: number) => void; unit?: string }> = ({ label, value, min, max, step = 1, onChange, unit = 'px' }) => (
    <div>
      ...
      <input id={label} type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(step === 1 ? parseInt(e.target.value) : parseFloat(e.target.value))} className="w-full mt-1 h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer"/>
    </div>
);
       ...
       <div className="flex-grow flex flex-col overflow-hidden">
            <div className="flex-shrink-0 border-b-2 border-indigo-500">
                <nav className="flex space-x-2 px-6">
                    <button onClick={() => setActiveTab('text')} className={`px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'text' ? 'text-white border-b-2 border-white' : 'text-slate-400 hover:text-white'}`}>TEXT</button>
                    <button onClick={() => setActiveTab('bubble')} className={`px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'bubble' ? 'text-white border-b-2 border-white' : 'text-slate-400 hover:text-white'}`}>BUBBLE</button>
                </nav>
            </div>
                        ...
                        <Section title="Content & Font">
                           <RichTextEditor key={bubbleProps.id} value={bubbleProps.text} onChange={(newText) => onUpdate({ text: newText })}/>
                            <div>
                                ...
                                <select id="font-family" value={bubbleProps.fontFamily} onChange={(e) => onUpdate({ fontFamily: e.target.value })} className="w-full mt-1 p-2 bg-slate-700 border border-slate-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                            ...
                            <ColorInput label="Default Text Color" value={bubbleProps.textColor} onChange={(v) => onUpdate({ textColor: v })} />
                             <div>
                                ...
                                <div className="flex mt-1 space-x-1 bg-slate-700 p-1 rounded-md">
                                    {TEXT_ALIGN_OPTIONS.map(({ value, label }) => (
                                    <button key={value} onClick={() => onUpdate({ textAlign: value })} className={`w-full py-1 rounded transition-colors text-sm ${ bubbleProps.textAlign === value ? 'bg-indigo-600 text-white' : 'bg-slate-700 hover:bg-slate-600' }`}>
                                        {label}
                                    </button>
                                    ))}
                                </div>
                        ...
                        <Section title="Text Transform">
                            <div className="space-y-3 p-3 bg-slate-800 rounded-lg">
                                ...
                                <button
                                    ...
                                    className="w-full mt-2 flex items-center justify-center space-x-2 px-4 py-2 bg-slate-600 hover:bg-slate-500 rounded-md transition-colors text-sm"
                                >
                    ...
                    <div id="bubble-panel">
                        <Section title="Shape & Style">
                             ...
                             {bubbleProps.shape === ShapeType.FREEHAND && (
                                <div className="space-y-3 p-3 bg-slate-800 rounded-lg">
                                    ...
                                    <button
                                        ...
                                        className="w-full flex items-center justify-center space-x-2 px-4 py-2 bg-slate-600 hover:bg-slate-500 rounded-md transition-colors text-sm"
                                    >
                                        <ResetIcon className="w-4 h-4" />
                            ...
                            <div>
                                ...
                                <select id="border-style" value={bubbleProps.borderStyle} onChange={(e) => onUpdate({ borderStyle: e.target.value as any })} className="w-full mt-1 p-2 bg-slate-700 border border-slate-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                        ...
                        <Section title="Tail & Transform">
                             <div>
                                ...
                                <input id="rotation" type="range" min="-180" max="180" value={bubbleProps.rotation} onChange={(e) => onUpdate({ rotation: parseInt(e.target.value) })} className="w-full mt-1 h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
                                <button onClick={() => onUpdate({ rotation: 0 })} className="p-1.5 hover:bg-slate-700 rounded-md" title="Reset Rotation">
                                    <ResetIcon className="w-4 h-4" />
                                </button>
                                </div>
                            </div>
                            ...
                            {bubbleProps.tailVisible && (
                                <div className="space-y-4 pt-3 border-t border-slate-700">
                                    ...
                                    <div key={tail.id} className="p-3 bg-slate-800 rounded-lg space-y-3">
                                        <div className="flex justify-between items-center">
                                            <h4 className="font-semibold text-slate-300">Tail {index + 1}</h4>
                                            <button
                                        ...
                                        <div>
                                            ...
                                            <select id={`tail-type-${tail.id}`} value={tail.type} onChange={e => handleUpdateTail(tail.id, { type: e.target.value as TailType })} className="w-full mt-1 p-2 bg-slate-700 border border-slate-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
